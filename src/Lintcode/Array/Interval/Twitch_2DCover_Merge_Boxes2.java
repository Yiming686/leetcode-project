...
public class Twitch_2DCover_Merge_Boxes2 {
...
static class VerticalLine {
    ...
    Interval interval;
    int addOrRemove;
	public VerticalLine(float pos, Interval interval, int addOrRemove) {
		super();
		this.pos = pos;
...
public static float IIDCover(Box[] boxes) {
	...
	for(Box box : boxes){
		verticalLines.add(new VerticalLine(box.left,  new Interval(box.lower,  box.upper),  1));
		verticalLines.add(new VerticalLine(box.right, new Interval(box.lower,  box.upper), -1));
	}
	...
	List<Interval> intervals = new ArrayList<Interval>();
	VerticalLine prev = verticalLines.get(0);
	intervals.add(prev.interval);
	float area = 0.0f;
	for(int i = 1; i < verticalLines.size(); i++){
		VerticalLine curr = verticalLines.get(i);
