...
//	想想涉及到2个数组，k一般必须大于等于1，当k=1时只会涉及到1个数组，这是一个特例
    public static double findMedianSortedArrays(int[] arr1, int[] arr2) {
        ...
        if (len % 2 == 1) {
            ...
            return (findKthSmallest(arr1, 0, arr2, 0, len / 2) 
                  + findKthSmallest(arr1, 0, arr2, 0, len / 2 + 1)) / 2.0;
        }
    }
    
    public static int findKthSmallest(int[] arr1, int start1,
                              int[] arr2, int start2,
                              int k){	
...
//		此处保证两个数组都有第k/2个点，下面比较和删除，才有了意义
    ...
    //从arr1的start1位置和arr2的start2的位置开始，查找从小到大的第k个元素，也就第k小的元素
    //那么找第k大的元素呢？    
    public static int findKthSmallest12(int[] arr1, int start1,
                              int[] arr2, int start2,
                              int k){		
		if (start1 >= arr1.length) {
			return arr2[start2 + k - 1];
        ...
        //以下代码对k>=2才有效，这是前提条件		
		//取第一个数组的第k/2个元素		
		int val1 = start1 + k / 2 - 1 < arr1.length
		            ? arr1[start1 + k / 2 - 1]
		            : Integer.MAX_VALUE;
		int val2 = start2 + k / 2 - 1 < arr2.length
		            ? arr2[start2 + k / 2 - 1]
		            : Integer.MAX_VALUE; 
