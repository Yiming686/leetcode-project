...
//		board[2] = "dcan".toCharArray();
    ...
    public static void search2(char[][] board, int x, int y, TrieNode root, ArrayList<String> ans){
		...
		search2(board, x+1, y,   root.subtreeMap.get(ch), ans);
		search2(board, x,   y+1, root.subtreeMap.get(ch), ans);
		search2(board, x-1, y,   root.subtreeMap.get(ch), ans);
		search2(board, x,   y-1, root.subtreeMap.get(ch), ans);
    ...
    static class TrieNode {
		String str;
		 boolean hasWord;
		 HashMap<Character, TrieNode> subtreeMap;
		 public TrieNode() {
			 hasWord = false;
			 subtreeMap = new HashMap<Character, TrieNode>();
			 str = "";
		 }
	};
    ...
    public static TrieNode buildTrieTree(ArrayList<String> words){
		...
		for(String word : words){
			...
			for (int i = 0; i < word.length(); i++) {
				if (!now.subtreeMap.containsKey(word.charAt(i))) {
				    //核心操作，此for loop就是想干这个事
					now.subtreeMap.put(word.charAt(i), new TrieNode());
				}
				now  =  now.subtreeMap.get(word.charAt(i));
			}
...
// 		if(root.subtreeMap.containsKey(board[x][y])){
		for(int i = 0; i < 4; i++){
			...
			chList.add(ch+"");
			board[x][y] = 0;
			search(board, x+dx[i], y+dy[i],dx, dy, root.subtreeMap.get(ch), ans);
			board[x][y] = ch;
		}
// 		}
	}
	...
	public static void search2(char[][] board, int x, int y, int []dx, int []dy,TrieNode root, ArrayList<String> ans){
		...
		for(int i = 0; i < 4; i++){
		    ...
		    if(newX >= 0 && newX < board.length && newY >= 0 && newY < board[0].length && board[newX][newY] != 0 ){
    		    ...
    		    chList.add(ch+"");
    			board[newX][newY] = 0;
    			search2(board, newX, newY,dx, dy, root.subtreeMap.get(ch), ans);
    			board[newX][newY] = ch;		    
		    }
