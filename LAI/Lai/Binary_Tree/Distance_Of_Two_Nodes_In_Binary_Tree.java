...
//		int dis = distance(root, 1, 8, tp);TP.build("root", null, dis, toStr(root));//进来再加，
    ...
    public static int distance(TreeNode<Integer> root, int k1, int k2, TP tp) {
        int[] depth = new int[3];
        TreeNode node = distance(depth, 0, root, k1, k2, tp);TP.build("root", null, StringUtils.toStr(node), toStr(depth), 0, StringUtils.toStr(root));//进来再加，
        tp.print();
